...
def get_model_pred(model, graph, data_loader, sampler, args):
    ...
    with torch.no_grad():
        for node_idx in data_loader:
            blocks = sampler.sample_blocks(graph, node_idx.to(args['device']))
...
if __name__ == '__main__':
    ...
    # 支持 device 字段为 'cpu' 或 int
    if str(args['device']).lower() == 'cpu':
        args['device'] = torch.device('cpu')
    elif torch.cuda.is_available():
        args['device'] = torch.device('cuda:%d' % int(args['device']))
    else:
        args['device'] = torch.device('cpu')
    ...
    # === 新增：保存AUROC、AUPRC、Rec@K到CSV ===
    import csv
    import os
    # 假设mean_results[0]=AUROC, mean_results[1]=AUPRC, mean_results[2]=Rec@K
    auroc = mean_results[0]
    auprc = mean_results[1]
    rec_at_k = mean_results[2]
    dataset_name = args['data-set']
    csv_file = 'results_summary.csv'
    file_exists = os.path.isfile(csv_file)
    with open(csv_file, 'a', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(['dataset', 'AUROC', 'AUPRC', 'Rec@K'])
        writer.writerow([dataset_name, auroc, auprc, rec_at_k])
    print(f'已将结果保存到 {csv_file}')
